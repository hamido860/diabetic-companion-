...
const EditableField: React.FC<{
  ...
  unit: string;
  onInfoChange: (field: keyof Omit<NutritionInfo, 'recipeName' | 'confidence'>, value: string) => void;
}> = ({ label, value, Icon, color, field, unit, onInfoChange }) => {
  ...
  return (
    <div className={`${styles.bg} p-4 rounded-xl flex items-center justify-between`}>
      <div className="flex items-center gap-4">
        ...
        <label htmlFor={field} className={`font-semibold text-md ${styles.text}`}>
          {label}
        </label>
      </div>
...
const NutritionResult: React.FC<{
  info: NutritionInfo;
  onLogMeal: (info: NutritionInfo) => void;
  onReset: () => void;
  isLogged: boolean;
  error: string | null;
}> = ({ info, onLogMeal, onReset, isLogged, error }) => {
  ...
  return (
    <div className="bg-brand-olive p-6 rounded-md shadow-lg shadow-black/20 animate-fadeInUp">
      ...
      <p className="text-sm text-brand-beige mb-4">
        ...
        <span
          className={`font-semibold ${
            editableInfo.confidence === 'High'
              ? 'text-green-400'
              : editableInfo.confidence === 'Medium'
              ? 'text-yellow-400'
              : 'text-red-400'
          }`}
        >
          {editableInfo.confidence}
        </span>
      </p>
      ...
      <div className="space-y-3 mb-6">
        <EditableField
          label={t('carbs')}
          value={editableInfo.carbohydrates}
          Icon={CubeIcon}
          color="teal"
          field="carbohydrates"
          unit="g"
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-4 mt-2 border-t border-white/10">
        <button
          onClick={() => onLogMeal(editableInfo)}
          disabled={isLogged}
...
const MealScanner: React.FC = () => {
  ...
  const handleLogMeal = (editableInfo: NutritionInfo) => {
      setError(null);
      try {
          addLoggedItem({
